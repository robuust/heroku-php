...
ARG PHP_VERSION=${PHP_MINOR_VERSION}.25
...
# PHP 8.4+ has version-specific defaults; older versions use the generic configuration
RUN PHP_CONFIG_PATH=$(php -r 'echo PHP_VERSION_ID >= 80400 ? PHP_MAJOR_VERSION . "/" . PHP_MINOR_VERSION . "/" : "";') && \
  curl --fail --silent --show-error --location "https://raw.githubusercontent.com/heroku/heroku-buildpack-php/main/support/build/conf/php/${PHP_CONFIG_PATH}conf.d/000-heroku.ini" > /app/.heroku/php/etc/php/php.ini
